...
close all;
%% 骨架和提取骨架后得到的矩阵啊之类的
formatSpec = 'D:/GitHub/data/multi/%s/mesh_%.4d.ply';
P_formatSpec = 'D:/GitHub/data/multi/%s/mesh_%.4d_skeleton';
% filename = 'lioness/lioness_ske';
% filenameOut = 'lioness/lioness_ske_aligned';
filename = 'Cats/cat_ske';
filenameOut = 'Cats/cat_ske_aligned';
% filename = 'Dogs/dog_ske';
% filenameOut = 'Dogs/dog_ske_aligned';
idx = [1];
...
xyz_models = cell(N,1);
for ii = idx
    fprintf('segmenting model %d\n', ii);
    paths = sprintf(formatSpec, filename, ii);
    P_path = sprintf(P_formatSpec, filename, ii);
    ...
    [importantpts, parts, legslength]  = keypts_horse(P.spls, P, 1);
...
for ii = 1:cnt_models
    ...
    path_write = sprintf(formatSpec, filenameOut,idx(ii));
